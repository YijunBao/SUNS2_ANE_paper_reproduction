...
patch_size = 160;
list_thred_ratio = 20;
% if contains(method,'CNMFE')
%     dir_traces = fullfile(path_name,'CNMFE\cv_save_20230715');
% else
%     dir_traces = fullfile(path_name,'min1pipe\cv_save_20230716');
% end

list_spike_type = {'include'}; % {'only','include','exclude'};
% spike_type = 'exclude'; % {'include','exclude','only'};
list_sigma_from = {'Unmix'}; % {'Raw','Unmix'}; 
dir_label = fullfile(path_name,'GT transients');
dir_GT_masks = fullfile(path_name,'GT Masks'); % FinalMasks_
dir_traces_GT = fullfile(dir_GT_masks, 'TUnCaT/raw');
dir_traces_out = fullfile(path_name, 'DeepWonder_scale_full/TUnCaT/raw');
...
% %                 list_thred_ratio=20:10:100; % 6:0.5:9; % 8:2:20; % 
%             end
...
%             list_alpha = 1; %
            ...
            if useTF
                dFF = h5read(fullfile(path_name,'1P_spike_tempolate.h5'),'/filter_tempolate')';
                dFF = dFF(dFF>exp(-1));
                dFF = dFF'/sum(dFF);
                kernel=fliplr(dFF);
            else
            ...
            for ii = 1:num_Exp
                ...
                load(fullfile(dir_traces_out,[Exp_ID,'.mat']),'traces', 'bgtraces');
                if useTF
                ...
                load(sprintf('%s\\mat\\%s.mat', target_dir,filename),'network_A_filt','network_C_filt');

                % load(fullfile(dir_traces,[Exp_ID,'_Masks.mat']),'network_A_filt','network_C_filt');
                Masks3 = permute(network_A_filt,[2,1,3]);
                traces_unmixed = network_C_filt;
                if useTF
...
%             save(fullfile(dir_parent,sprintf('Transient_accuracy_%s.mat',method)),...
%                 'list_recall_match','list_precision_match','list_F1_match',...
