function [recall, precision, F1,spikes_GT_array,spikes_eval_array] ...
    = GetPerformance_SpikeDetection_unmatch_weight(output,traces_eval,m_spatial,thred_ratio,sigma,mu, traces_GT_norm,traces_norm_out)
...
for nn=1:num_GT
    if ~isempty(output{ind_hasGT(nn)})
        trace_GT=traces_GT_norm(nn,:);
        output_temp=output{ind_hasGT(nn)};
        output_temp=output_temp(logical(output_temp(:,3)),1:2);
        output_temp(output_temp(:,1) > output_temp(:,2),:) = []; 
%         output_temp(:,2) = output_temp(:,2) - 1;
        spikes_GT_array{nn}=[output_temp,zeros(size(output_temp,1),2)];
        for ii=1:size(output_temp,1)
            spikes_GT_line(nn,output_temp(ii,1):output_temp(ii,2))=true;
            spikes_GT_array{nn}(ii,4) = max(trace_GT(output_temp(ii,1):output_temp(ii,2)));
        end
    end
end
...
spikes_eval_array=cell(n_seg,1);
for nn=1:n_seg
...
%             temp_spikes_eval_array(ii,3)=0;
            temp_spikes_split = zeros(0,4);
        elseif num_spikes==1
%             temp_spikes_eval_array(ii,3)=0;
            temp_spikes_split = [starts(ii),ends(ii),0,0];
        elseif num_spikes>1
%             multi_spike = trace(starts(ii):ends(ii));
        ...
        temp_spikes_split(temp_spikes_split(:,1) > temp_spikes_split(:,2),:) = []; 
        for jj = 1:size(temp_spikes_split,1)
            temp_spikes_split(jj,4) = max(trace_out(temp_spikes_split(jj,1):temp_spikes_split(jj,2)));
        end
...
m_spatial = m_spatial(hasGT,:);
[select_seg, select_GT] = find(m_spatial');
...
n_eval_match=zeros(n_seg,1);
for nn=1:n_match
    nn_GT = select_GT(nn);
    nn_seg = select_seg(nn);
%     spikes_GT1=spikes_GT_line(nn,:);
    spikes_GT2=spikes_GT_array{nn_GT};
    n_GT = size(spikes_GT2,1);
%     spikes_eval1=spikes_eval_line(nn,:);
    spikes_eval2=spikes_eval_array{nn_seg};
...
%     end
    GT_match = any(m,2);
    n_GT_match(nn_GT)=sum(GT_match);
    if n_GT_match(nn_GT)
        spikes_GT_array{nn_GT}(:,3)=GT_match.*spikes_GT_array{nn_GT}(:,4);
    end
...
%     end
    trace_GT = traces_GT_norm(nn_GT,:);
    spikes_eval_temp = spikes_eval_array{nn_seg};
    for jj = 1:size(spikes_eval_temp,1)
        spikes_eval_temp(jj,4) = max(trace_GT(spikes_eval_temp(jj,1):spikes_eval_temp(jj,2)));
    end
    spikes_eval_array{nn_seg} = spikes_eval_temp;
    eval_match = any(m,1)';
    n_eval_match(nn_seg)=sum(eval_match);
    if n_eval_match(nn_seg)
        spikes_eval_array{nn_seg}(:,3)=eval_match.*spikes_eval_array{nn_seg}(:,4);
    end
end
    
% n_GT_match = cellfun(@(x) sum(x(:,3)), spikes_GT_array);
% n_GT_total = cellfun(@(x) sum(x(:,4)), spikes_GT_array);
% n_eval_match = cellfun(@(x) sum(x(:,3)), spikes_eval_array);
% n_eval_total = cellfun(@(x) sum(x(:,4)), spikes_eval_array);
for nn = 1:size(spikes_GT_array,1)
    if ~isempty(spikes_GT_array{nn})
        n_GT_match(nn) = sum(spikes_GT_array{nn}(:,3));
        n_GT_total(nn) = sum(spikes_GT_array{nn}(:,4));
    end
end
for nn = 1:size(spikes_eval_array,1)
    if ~isempty(spikes_eval_array{nn})
        n_eval_match(nn) = sum(spikes_eval_array{nn}(:,3));
        n_eval_total(nn) = sum(spikes_eval_array{nn}(:,4));
    end
end
