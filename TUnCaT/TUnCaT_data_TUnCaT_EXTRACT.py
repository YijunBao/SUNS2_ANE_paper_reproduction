...
if __name__ == '__main__':
    ...
    for Exp_ID in list_Exp_ID:
        ...
        filename_masks = os.path.join(dir_masks, Exp_ID + '_EXTRACT.mat')
        filename_masks_resave = os.path.join(dir_traces, 'Output_Masks_' + Exp_ID + '.mat')
        
        try:
            file_masks = loadmat(filename_masks)
            Masks = file_masks['output']['spatial_weights']
            Masks = (Masks>0.2).astype('bool')
        except:
            file_masks = h5py.File(filename_masks, 'r')
            Masks = np.array(file_masks['output']['spatial_weights']).transpose([2,1,0])
            Masks = (Masks>0.2).astype('bool')
            file_masks.close()
