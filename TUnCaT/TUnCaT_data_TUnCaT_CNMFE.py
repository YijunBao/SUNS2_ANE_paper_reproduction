...
if __name__ == '__main__':
    ...
    # dir_masks = 'CNMFE\\cv_save_20230715'
    dir_masks = 'min1pipe\cv_save_20230716'
    ...
    for Exp_ID in list_Exp_ID:
        ...
        filename_masks = os.path.join(dir_masks, Exp_ID + '_Masks.mat')
        filename_masks_resave = os.path.join(dir_traces, 'Output_Masks_' + Exp_ID + '.mat')
        
        try:
            file_masks = loadmat(filename_masks)
            Masks = file_masks['Masks3'].astype('bool')
        except:
            file_masks = h5py.File(filename_masks, 'r')
            Masks = np.array(file_masks['Masks3']).transpose([2,1,0]).astype('bool')
            file_masks.close()
