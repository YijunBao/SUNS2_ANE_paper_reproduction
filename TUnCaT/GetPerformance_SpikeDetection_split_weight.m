function [recall, precision, F1,individual_recall,individual_precision,...
    ...
    = GetPerformance_SpikeDetection_split_weight(output,traces_eval,thred_ratio,sigma,mu, traces_norm)
...
%             multi_spike = trace(starts(ii):ends(ii));
            ...
            temp_spikes_split = zeros(num_spikes,4);
        ...
        for jj = 1:size(temp_spikes_split,1)
            temp_spikes_split(jj,4) = max(trace_GT(temp_spikes_split(jj,1):temp_spikes_split(jj,2)));
        end
...
%     end
    ...
    if n_GT_match(nn)
        spikes_GT_array{nn}(:,3)=GT_match.*spikes_GT_array{nn}(:,4);
    end
...
%     end
    ...
    if n_eval_match(nn)
        spikes_eval_array{nn}(:,3)=eval_match.*spikes_eval_array{nn}(:,4);
    end
end
