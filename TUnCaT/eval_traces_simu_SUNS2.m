...
dir_SUNS = fullfile(path_name, dir_SUNS_sub);
dir_masks = fullfile(dir_SUNS, 'output_masks/TUnCaT');
...
for cv = 1:num_Exp
    for eid = cv % trains % 1:num_Exp
        ...
        load(fullfile(dir_masks,['Output_Masks_',Exp_ID,'.mat']),'Masks','traces');
        Masks3 = Masks;
        traces = traces';
...
save(fullfile(dir_masks,'mean_corr.mat'),'list_crop','mean_corr','mean_corr_match','Table_mean_corr','Table_mean_corr_match');
