...
for k=1:num_Exp
    ...
    for method = 1:num_method
        ...
        PSNR = max(SNR,[],2)';
%         load([dir_output_mask,Exp_ID,'_traces.mat'],'traces_SNR','traces_demix');
%         med = median(traces_demix,2);
%         sigma = median(abs(traces_demix - med))/(sqrt(2)*erfinv(1/2));
%         SNR = (traces_demix - med)./sigma;
%         PSNR = max(SNR,[],2)';
...
%%
save(['Precision_PSNR ',data_name,' ',num2str(threshJ),' 0607.mat'],'Nfind_PSNR','N_PSNR');
% %%
...
%% Precision vs PSNR
% load(['Precision_PSNR ',data_name,' ',num2str(threshJ),'.mat'],'Nfind_PSNR','N_PSNR');
...
saveas(gcf,['Precision vs PSNR ',addon,data_name,' ',num2str(threshJ),' 0607.png'])
...
saveas(gcf,['Number vs PSNR ',addon,data_name,' ',num2str(threshJ),' 0607.png'])
...
saveas(gcf,['FP Number vs PSNR ',addon,data_name,' ',num2str(threshJ),' 0607.png'])
