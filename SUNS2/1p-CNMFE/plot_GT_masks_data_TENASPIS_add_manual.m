...
dir_original = 'D:\data_TENASPIS\original_masks\';
% dir_video = 'D:\data_TENASPIS\added_blockwise_weighted_sum_unmask';
dir_video = dir_original;
...
dir_GT_masks = fullfile(dir_video,'added_blockwise\GT Masks');
...
%     edge_masks_add2_sum_mag=kron(edge_masks_add2_sum,mag_kernel_bool);
    ...
    % figure(98)
    % subplot(2,3,1)
    figure('Position',[50,50,700,600],'Color','w');
...
%     rectangle('Position',[3,65,5,26],'FaceColor','w','LineStyle','None'); % 20 um scale bar
    ...
    if save_figures
        saveas(gcf,[Exp_ID, '_added_blockwise masks.png']);
