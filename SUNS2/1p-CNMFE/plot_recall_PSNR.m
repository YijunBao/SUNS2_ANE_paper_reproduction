...
%                     pix_select_sigthres, pix_select_corrthres, merge_roi_corrthres);
        ...
        TP_2 = sum(m,2)>0;
        for ii = 1:Lbin
            in_range = TP_2(select_SNR(:,ii));
            N_PSNR(ii,method,cv) = sum(in_range);
        end
    end
end

save(['recall_PSNR ',data_name,' ',num2str(threshJ),' 0607.mat'],'NGT_PSNR','N_PSNR');
% %%
...
NGT_PSNR_all = sum(NGT_PSNR,2);
N_PSNR_all = sum(N_PSNR,3);
% max_PSNR = 60;
max_PSNR = max(range_SNR);

bin=1;
NGT_PSNR_all = sum(reshape(NGT_PSNR_all,bin,[]),1)';
N_PSNR_all = reshape(sum(reshape(N_PSNR_all,bin,[]),1),[],num_method);
recall_PSNR_mean = N_PSNR_all./NGT_PSNR_all;
...
hold on;
plot(range_SNR_show(1:end),recall_PSNR_mean(1:end,1),'Color',color(1,:),'LineWidth',2,'Marker','.','MarkerSize',15);
plot(range_SNR_show(1:end),recall_PSNR_mean(1:end,2),'Color',color(2,:),'LineWidth',2,'Marker','.','MarkerSize',15);
plot(range_SNR_show(1:end),recall_PSNR_mean(1:end,3),'Color',color(4,:),'LineWidth',2,'Marker','.','MarkerSize',15);
plot(range_SNR_show(1:end),recall_PSNR_mean(1:end,4),'Color',color(5,:),'LineWidth',2,'Marker','.','MarkerSize',15);
...
saveas(gcf,['Recall vs PSNR ',addon,data_name,' ',num2str(threshJ),' 0607.png'])

%% Plot number of neurons vs. PSNR
% inset=axes('Position',[0.6,0.2,0.3,0.3]);
figure('Position',[550,50,500,500],'color','w');
bar(range_SNR_show,NGT_PSNR_all); %,'LineWidth',2
% xlabel('PSNR');
...
saveas(gcf,['GT Number vs PSNR ',addon,data_name,' ',num2str(threshJ),' 0607.png'])
