...
from suns.run_suns_video_pmap import suns_online_track
...
if __name__ == '__main__':
    sub_folder = '4816[1]th6' # sys.argv[1]# + '+BGlayer'
    # %% setting parameters
    rate_hz = 20 # frame rate of the video
    Dimens = (50,50) # lateral dimensions of the video
    nframes = 6000 # number of frames used for preprocessing. 
    ...
    dir_parent = dir_video + 'complete_TUnCaT' # folder to save all the processed data
    dir_sub = 'test_CNN\\' + sub_folder
    dir_output = os.path.join(dir_parent, dir_sub, 'output_masks online track video pmap2') # folder to save the segmented masks and the performance scores
    dir_params = os.path.join(dir_parent, dir_sub, 'output_masks') # folder of the optimized hyper-parameters
    weights_path = os.path.join(dir_parent, dir_sub, 'Weights') # folder of the trained CNN
    if not os.path.exists(dir_output):
        os.makedirs(dir_output) 

    # %% pre-processing parameters
    nn = nframes
    gauss_filt_size = 25*Mag # standard deviation of the spatial Gaussian filter in pixels
    ...
    filename_TF_template = dir_video + '1P_spike_tempolate.h5'
    ...
    for CV in list_CV:
        ...
        Masks, Masks_2, time_total, time_frame, list_time_per, list_Masks_cons_2D, list_active_old, \
            video_input_all, pmaps_b_all = suns_online_track(
        ...
        savemat(os.path.join(dir_output, 'Output_Masks_{}.mat'.format(Exp_ID)), {'Masks':Masks, 'list_time_per':list_time_per,\
            'list_Masks_cons_2D': list_Masks_cons_2D, 'list_active_old':list_active_old, \
            'video_input_all':video_input_all, 'pmaps_b_all':pmaps_b_all}, do_compression=True)
