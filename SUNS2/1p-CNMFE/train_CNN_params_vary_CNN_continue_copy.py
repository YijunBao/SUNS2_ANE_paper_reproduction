...
if __name__ == '__main__':
    sys.argv = ['py', '3', '4', '[1,2]', 'elu', 'True', '4816[1,2]th4', '4', '0']
    ...
    max_eid = int(sys.argv[8])

    # %% setting parameters
    rate_hz = 20 # frame rate of the video
    Dimens = (424,232) # lateral dimensions of the video
    nn = 37000 # number of frames used for preprocessing. 
    ...
    num_train_per = 200*4 # Number of frames per video used for training 
    ...
    list_thred_ratio = list(range(3,7)) # [thred_std] # A list of SNR threshold used to determine when neurons are active.
    ...
    num_total = 35000 # nn - Poisson_filt.size + 1
    ...
    list_minArea = [25] # list(range(20,60,5)) # 
    ...
    list_thresh_pmap = [120] # list(range(100,166,20)) # 
    ...
    parameter_optimization_cross_validation(cross_validation, list_Exp_ID, Params_set, \
        ...
        batch_size_eval, useWT=useWT, useMP=False, load_exist=load_exist, max_eid=max_eid, \
        n_depth=n_depth, n_channel=n_channel, skip=skip, activation=activation, double=double) # 
