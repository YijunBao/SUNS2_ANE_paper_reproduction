...
save_folder = '.\plot pipeline MF twice\';
...
[image_new_crop, mask_new_crop, mask_new_full, select_frames_class, select_weight_calss, ...
    ...
    find_missing_blockwise_weighted_sum_unmask_class(video, masks, xmin, xmax, ymin, ymax, ...
...
%         text(yy,xx,num2str(classes(kk)),'FontSize',18,'Color','y');
        text(yy,xx,[num2str(classes(kk)),':  ',num2str(select_frames_sort_full(kk))],'FontSize',14,'Color','y');
    else
%         text(yy,xx,'X','FontSize',18,'Color','y');
        text(yy,xx,['X:  ',num2str(select_frames_sort_full(kk))],'FontSize',14,'Color','y');
    end
end
% %% 
