...
% SNR_range = [2,10]; % [2,10]; % [2.5,5]; % [3,4]; % 
SNR_range = [0,1000]; % [2,10]; % [2.5,5]; % [3,4]; % 
addpath(genpath('C:\Matlab Files\STNeuroNet-master\Software'))

%% neurons and masks frame
list_data_names={'lowBG=5e+03,poisson=1'};
num_Exp = 10;
...
%     raw_max = max(video_raw,[],3) - min(video_raw,[],3);
    ...
    %% magnify
    mag=2;
    mag_kernel = ones(mag,mag,class(raw_max));
    mag_kernel_bool = logical(mag_kernel);
    SNR_max_mag = kron(raw_max,mag_kernel);
...
%     rectangle('Position',[3,65,5,26],'FaceColor','w','LineStyle','None'); % 20 um scale bar
    ...
    if save_figures
        saveas(gcf,[data_name, ' GT Masks ',list_title{k},'_raw.png']);
