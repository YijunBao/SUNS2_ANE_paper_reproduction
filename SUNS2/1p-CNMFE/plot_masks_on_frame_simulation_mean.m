...
saved_date_MIN1PIPE = '20220723';
saved_date_CNMFE = '20220721'; % '20220809 cv';
mag=1;
mag_crop=4;
%%
for k=1:num_Exp
...
% N_neuron2 = 2; % 
% N_neuron3 = 3; % 
% rect1 = [1,1,2,2]; rect2 = rect1; rect3 = rect1; 
% % rect1=[30,40,30,40];
% % rect2=[30,40,30,40];
% % rect3=[30,40,30,40];
...
%% SUNS + missing finder
% dir_output_mask_SUNS = fullfile(dir_video,'complete_TUnCaT\4816[1]th4\output_masks');
...
%     set(h,'FontSize',12);
% end
% rectangle('Position',rect1_sub,'EdgeColor',color(7,:),'LineWidth',1);
% rectangle('Position',rect2_sub,'EdgeColor',color(7,:),'LineWidth',1);
% rectangle('Position',rect3_sub,'EdgeColor',color(7,:),'LineWidth',1);
...
for n = 1:NGT
    contour(GT_Masks_mag(xrange,yrange,n), 'EdgeColor',color(3,:),'LineWidth',0.5);
end
for n = 1:N_SUNS
    contour(Masks_SUNS_mag(xrange,yrange,n), 'EdgeColor',color(5,:),'LineWidth',0.5);
end
% alphaImg = ones(Lxc_mag,Lyc_mag).*reshape(color(3,:),1,1,3);
% image(alphaImg,'Alphadata',alpha*(edge_GT_Masks_sum_mag(xrange_mag,yrange_mag)));  
% alphaImg = ones(Lx*mag,Ly*mag).*reshape(colors_multi(1,:),1,1,3);
...
if save_figures
    if image_only
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks SUNS_TUnCaT ',list_title{k},' ',mat2str(SNR_range),'.tif']);
    else
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks SUNS_TUnCaT ',list_title{k},' ',mat2str(SNR_range),'.png']);
    end
    % saveas(gcf,['figure 2\',Exp_ID,' SUNS noSF h.svg']);
end
...
for n = 1:N_SUNS
    contour(Masks_SUNS_mag(xrange,yrange,n), 'EdgeColor',color(4,:),'LineWidth',0.5);
end
% alphaImg = ones(Lxc_mag,Lyc_mag).*reshape(color(3,:),1,1,3);
% image(alphaImg,'Alphadata',alpha*(edge_GT_Masks_sum_mag(xrange_mag,yrange_mag)));  
...
if save_figures
    if image_only
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks SUNS_FISSA ',list_title{k},' ',mat2str(SNR_range),'.tif']);
    else
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks SUNS_FISSA ',list_title{k},' ',mat2str(SNR_range),'.png']);
    end
    % saveas(gcf,['figure 2\',Exp_ID,' SUNS noSF h.svg']);
end
...
for n = 1:N_min1
    contour(Masks_min1_mag(xrange,yrange,n), 'EdgeColor',color(1,:),'LineWidth',0.5);
end
% alphaImg = ones(Lxc_mag,Lyc_mag).*reshape(color(3,:),1,1,3);
% image(alphaImg,'Alphadata',alpha*(edge_GT_Masks_sum_mag(xrange_mag,yrange_mag)));  
...
if save_figures
    if image_only
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks min1pipe ',list_title{k},' ',mat2str(SNR_range),'.tif']);
    else
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks min1pipe ',list_title{k},' ',mat2str(SNR_range),'.png']);
    end
...
for n = 1:N_cnmfe
    contour(Masks_cnmfe_mag(xrange,yrange,n), 'EdgeColor',color(2,:),'LineWidth',0.5);
end
% alphaImg = ones(Lxc_mag,Lyc_mag).*reshape(color(3,:),1,1,3);
% image(alphaImg,'Alphadata',alpha*(edge_GT_Masks_sum_mag(xrange_mag,yrange_mag)));  
...
if save_figures
    if image_only
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks CNMF-E ',list_title{k},' ',mat2str(SNR_range),'.tif']);
    else
        saveas(gcf,[save_folder, data_name,' ',Exp_ID, ' Masks CNMF-E ',list_title{k},' ',mat2str(SNR_range),'.png']);
    end
    % saveas(gcf,['figure 2\',Exp_ID,' CaImAn Batch h.svg']);
end
