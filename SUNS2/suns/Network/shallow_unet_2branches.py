...
def get_shallow_unet_more(size=None, n_depth=3, n_channel=4, skip=[1], activation='elu', Params_loss=None):
    ...
    # background layer
    c01 = tf.keras.layers.Conv2D(n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(inputs)
    do01 = tf.keras.layers.Dropout(0.1)(c01)

    mp01 = tf.keras.layers.MaxPooling2D((2, 2))(do01)
    c02 = tf.keras.layers.Conv2D(2*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(mp01)
    do02 = tf.keras.layers.Dropout(0.1)(c02)

    if n_depth>2:
        mp02 = tf.keras.layers.MaxPooling2D((2, 2))(do02)
        c03 = tf.keras.layers.Conv2D(4*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(mp02)
        do03 = tf.keras.layers.Dropout(0.2)(c03)

        if n_depth>3:
            mp03 = tf.keras.layers.MaxPooling2D((2, 2))(do03)
            c04 = tf.keras.layers.Conv2D(8*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(mp03)
            do04 = tf.keras.layers.Dropout(0.2)(c04)

            c04 = tf.keras.layers.Conv2D(8*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do04)
            do04 = tf.keras.layers.Dropout(0.2)(c04)

            ct03 = tf.keras.layers.Conv2DTranspose(4*n_channel, (2, 2), strides=(2, 2), padding='same')(do04)
            if 3 in skip:
                ct03 = tf.keras.layers.concatenate([ct03, do03], axis=3)
            c03 = tf.keras.layers.Conv2D(4*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(ct03)
            do03 = tf.keras.layers.Dropout(0.2)(c03)

        else:
            c03 = tf.keras.layers.Conv2D(4*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do03)
            do03 = tf.keras.layers.Dropout(0.2)(c03)

        ct02 = tf.keras.layers.Conv2DTranspose(2*n_channel, (2, 2), strides=(2, 2), padding='same')(do03)
        if 2 in skip:
            ct02 = tf.keras.layers.concatenate([ct02, do02], axis=3)
        c02 = tf.keras.layers.Conv2D(2*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(ct02)
        do02 = tf.keras.layers.Dropout(0.1)(c02)

    else:
        c02 = tf.keras.layers.Conv2D(2*n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do02)
        do02 = tf.keras.layers.Dropout(0.1)(c02)

    ct01 = tf.keras.layers.Conv2DTranspose(n_channel, (2, 2), strides=(2, 2), padding='same')(do02)
    if 1 in skip:
        ct01 = tf.keras.layers.concatenate([ct01, do01], axis=3)

    c01 = tf.keras.layers.Conv2D(n_channel, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(ct01)
    do01 = tf.keras.layers.Dropout(0.1)(c01)

    # neuron minus background layer as output
    diff = do1 - do01
    outputs = tf.keras.layers.Conv2D(1, (1, 1), activation='sigmoid')(diff)
