...
def generate_masks(network_input:np.array, file_mask:str, list_thred_ratio:list, dir_save:str, Exp_ID:str):
    ...
    med_raw = np.quantile(raw_traces, np.array([0.5, 0.75]), axis=1)
    med_unmix = np.quantile(unmixed_traces, np.array([0.5, 0.75]), axis=1)
    mu_unmix = med_unmix[0]
    sigma_raw = (med_raw[1]-med_raw[0])/(math.sqrt(2)*special.erfinv(0.5))
