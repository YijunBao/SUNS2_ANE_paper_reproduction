...
def suns_batch(dir_video, Exp_ID, filename_CNN, Params_pre, Params_post, dims, \
    ...
    fffsub = tf.keras.models.Model(inputs = fff.input, \
        outputs = [fff.get_layer('dropout_5').output, fff.get_layer('dropout_11').output])
    fffsub.compile(optimizer='adam', loss='mean_squared_error')
    ...
    # fffsub.evaluate(init_imgs, [init_masks,init_masks], batch_size=batch_size_eval)
    ...
    # inter = fffsub.predict(video_input, batch_size=batch_size_eval)
    # [foreground, background] = inter
    [foreground, background] = fffsub.predict(video_input, batch_size=batch_size_eval)
    if display:
    ...
    foreground = foreground.squeeze()[:, :Lx, :Ly]
    background = background.squeeze()[:, :Lx, :Ly]
    savemat(os.path.join(dir_video, Exp_ID+'_intermediate.mat'), {'prob_map':prob_map, 'foreground':foreground, 'background':background})
