...
def suns_online(filename_video, filename_CNN, Params_pre, Params_post, dims, \
    ...
    med_frame3, segs_all, recent_frames = init_online(
        bb, dims, dimspad, video_input, pmaps_b_init, fff, thresh_pmap_float, Params_post, \
    ...
    for t in range(frames_initf,nframesf):
        ...
        # PreProcessing
        frame_SNR, frame_tf = preprocess_online(bb, dimspad, dimspad, med_frame3, frame_SNR, \
...
def suns_online_track(filename_video, filename_CNN, Params_pre, Params_post, dims, \
    ...
    nframesf = nframes - leng_tf + 1
    bb[:, :Lx, :Ly] = video_raw[:frames_init]
    video_input_all = np.zeros((nframesf, rowspad, colspad), dtype='float32')        
    pmaps_b_all = np.zeros((nframesf, Lx, Ly), dtype='uint8')        
    if display:
        time_load = time.time()
        print('Load data: {} s'.format(time_load-time_init))
    ...
    for t in range(frames_initf, nframesf):
        ...
        # CNN inference
        frame_prob = CNN_online(frame_SNR, fff, dims)
        video_input_all[t] = frame_SNR
        pmaps_b_all[t] = frame_prob > thresh_pmap_float
    ...
    # convert to a 3D array of the segmented neurons
    Masks = np.reshape(Masks_2.toarray(), (Masks_2.shape[0], Lx, Ly)).astype('bool')
    return Masks, Masks_2, time_total, time_frame, list_time_per, list_Masks_cons_2D, list_active_old, video_input_all, pmaps_b_all
