...
v = VideoWriter(['Track Masks ',Exp_ID,'.avi']);
v.FrameRate = fps;
open(v);
...
%     set(h,'FontSize',9);
    ...
    hold on;
    if t > t_init
        mask_track = reshape(full(list_Masks_cons_2D{t-t_init}'),Lx,Ly,[]);
    end
    temp_mask = sum(mask_track(rangey,rangex,:),3)';
    temp_mask_mag = kron(temp_mask,mag_kernel);
    contour(temp_mask_mag,'Color', [0.9,0.1,0.1]);
    if t > t_init
        act_mask = sum(mask_track(rangey,rangex,list_active_old{t-t_init}'),3)';
