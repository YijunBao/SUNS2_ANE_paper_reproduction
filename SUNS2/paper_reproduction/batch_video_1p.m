...
load([dir_mask,'output_masks pmap\Output_Masks_',Exp_ID,'.mat'],'Masks','pmaps_b','times_active');
Masks = permute(Masks,[2,3,1]);
pmaps_b = permute(pmaps_b,[3,2,1]);
...
%%
mask = sum(Masks,3);
mask_mag = kron(mask,mag_kernel);
% border = 255*ones(Lxc,10,3,'uint8');
v = VideoWriter(['Batch Masks ',Exp_ID,'.avi']);
v.FrameRate = fps;
open(v);
...
%%    masks from SUNS batch
    clf;
    frame = image(rangey,rangex)';
    frame_mag = kron(double(frame),mag_kernel);
...
%     set(get(h,'Label'),'String','Raw intensity','FontName','Arial');
    ...
    if t>0
        ...
        temp_mask = sum(Masks(:,:,cells_active),3);
