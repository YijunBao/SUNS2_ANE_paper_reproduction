...
%     end
    ...
    %%
    load(fullfile(dir_GT_mask, ['FinalMasks_', Exp_ID, '_old.mat']), 'FinalMasks');
    FinalMasks_old = logical(FinalMasks);
    num_old = size(FinalMasks_old,3);
end

%%
figure;
histogram(PSNR(1:num_old));
hold on;
histogram(PSNR((num_old+1):end));
legend({'Original neurons','Added neurons'});
xlabel('PSNR');
ylabel('Numbers');
set(gca,'FontSize',14);
saveas(gcf,['GT Number vs PSNR old new.png'])

%%
[mean(PSNR(1:num_old)),mean(PSNR((num_old+1):end))]
