...
if __name__ == '__main__':
    list_name_video = ['amp=0.003,poisson=1']
    list_rate_hz = [10] # 
    Dimens = [(64,64)]
    list_nframes = [6000]
    num_Exp = 9
    ...
    num_train_per = 800 # Number of frames per video used for training 
    ...
    dir_video = os.path.join('E:\\simulation_constantBG_noise', name_video)
    ...
    list_thred_ratio = list(range(2,10)) # [thred_std] # A list of SNR threshold used to determine when neurons are active.
    ...
    list_minArea = list(range(20,85,10)) 
    # average area of a typical neuron (unit: pixels in ABO videos)
    list_avgArea = [91] # list(range(30,58,5)) # 
    # uint8 threshould of probablity map (uint8 variable, = float probablity * 256 - 1)
    list_thresh_pmap = list(range(60,256,20))
    # list_thresh_pmap = list(range(20,100,10))
    # list_thresh_pmap = list(range(50,130,10))
    # list_thresh_pmap = list(range(190,255,10)) 
    ...
    # list_thresh_COM = list(np.arange(3, 7, 0.5)) 
    ...
    #     # %% Determine active neurons in all frames
