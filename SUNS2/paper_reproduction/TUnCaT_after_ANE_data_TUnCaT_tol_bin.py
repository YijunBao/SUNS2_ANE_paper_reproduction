...
if __name__ == '__main__':
    ...
    sub_folder = 'add_new_blockwise/trained dropout 0.8exp(-15)' # sys.argv[3] # 
    ...
    dir_masks = os.path.join(dir_parent,dir_masks,sub_folder)
    ...
    for Exp_ID in list_Exp_ID:
        ...
        filename_masks = os.path.join(dir_masks, 'Output_Masks_' + Exp_ID + '_added.mat')
        filename_masks_resave = os.path.join(dir_traces, 'Output_Masks_' + Exp_ID + '.mat')
        
        try:
            file_masks = loadmat(filename_masks)
            Masks = file_masks['Masks'].astype('bool')
        except:
            file_masks = h5py.File(filename_masks, 'r')
            Masks = np.array(file_masks['Masks']).transpose([2,1,0]).astype('bool')
            file_masks.close()
