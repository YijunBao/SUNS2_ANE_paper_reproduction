...
list_scale_lowBG = [1e3,5e3];
list_scale_noise = [0.1,0.3,1];
...
for scale_lowBG = list_scale_lowBG
    for scale_noise = list_scale_noise
        ...
        figure;
        histogram(PSNR_all,0:2:40);
        xlabel('PSNR');
        ylabel('Numbers');
        text(0.7,0.6,{sprintf('Mean = %.1f',mean_PSNR),sprintf('Median = %.1f',median_PSNR)},...
            'Units','normalized','FontSize',14);
        data_name_split = split(data_name,' ');
        data_name_short = data_name_split{1};
        title(data_name_short, 'Interpreter','None')
        set(gca,'FontSize',14);
