...
gSig = list_avg_radius(data_ind)/2; % 3;
gSiz = 4*gSig+1;
...
for eid = 1:num_Exp
    ...
    %% Set parameters
    seed = eid;
%     K = round(100+10*randn); % bv;    % number of neurons
    K = round(60+5*randn); % PFC;    % number of neurons
    ...
    Ysiz = size(Y3); 
    h5create(video_h5,'/mov',Ysiz,'Datatype','uint16');
