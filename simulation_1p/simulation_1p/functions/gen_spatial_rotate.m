function [A, coor] = gen_spatial_rotate(K, siz, gSig, d_min, seed, elong)
%% simulate spatial components of K neurons within the field of view (siz(1) x siz(2) pixels).
%% input:
%   siz: [d1,d2,T] vector, dimension of the data
%   K: scalar, number of neurons
%   gSig: sigma for the 2D gaussian shape
%   d_min: scalar, minimum distance between two neurons
%   seed: scalar, seed number for generating random numbers

%% output:
%   A: d1*d2*K 3D matrix, spatial component
%   coor: K*2 matrix, center location of each neuron

%% Author: Pengcheng Zhou, Carnegie Mellon University, 2016

%% initialize parameters
if ~exist('d_min', 'var') || isempty(d_min)
    d_min = 5;
end
if ~exist('seed', 'var')||isempty(seed)
    seed = 1;
end
if ~exist('elong', 'var')||isempty(elong)
    elong = 0.1;
end
d1 = siz(1);
d2 = siz(2);
rng(seed);

%% generate spatial profiles for each neuron
% locations of the centers.
x0 = (rand(3*K,1)-0.5) * 0.8 *d2 + d2/2;  % some positions should be removed. thus initialize only 3K
y0 = (rand(3*K,1)-0.5) * 0.8 *d1 + d1/2;

while true
    % generate distances between neurons
    d_neurons = sqrt(bsxfun(@minus, x0, x0').^2+bsxfun(@minus, y0, y0').^2);
    [indr, indc] = find(d_neurons<d_min);
    ind = unique(indr(indr<indc));
    
    % remove pixels that are too close
    if isempty(ind)
        if length(x0)<K
            x0 = [x0; (rand(1*K,1)-0.5) * 0.8 *d2 + d2/2];  %#ok<*AGROW> % some positions should be removed. thus initialize only 3K
            y0 = [y0; (rand(1*K,1)-0.5) * 0.8 *d1 + d1/2];
        else
            break;
        end
    else
        x0(ind) = [];
        y0(ind) = [];
    end
end

x0 = x0(1:K);   % select only top K neurons
y0 = y0(1:K);

% choose sig_x and sig_y for each neuron
sig_x = exp(randn(K,1)*elong) * gSig;
sig_y = exp(randn(K,1)*elong) * gSig;
sig_x(or(sig_x<exp(-2*elong)*gSig, sig_x>exp(2*elong)*gSig)) = gSig;
sig_y(or(sig_y<exp(-2*elong)*gSig, sig_y>exp(2*elong)*gSig)) = gSig;
theta = unifrnd(-45,45);
[x, y] = meshgrid(1:d2, 1:d1);
A = zeros(d1,d2,K);
coor.x0 = round(x0);
coor.y0 = round(y0);
for m=1:K
    % initialize each neuron's spatial component
    temp1 = ((x-x0(m))/sig_x(m)).^2 + ((y-y0(m))/sig_y(m)).^2;
    temp0 = exp(-temp1/2);
    temp0(temp0<exp(-5)) = 0;    % remove boundaries
    
    temp1 = ((x-x0(m)+round(x0(m))-round(d2/2))/sig_x(m)).^2 + ((y-y0(m)+round(y0(m))-round(d1/2))/sig_y(m)).^2;
    temp12 = exp(-temp1/2);
    temp2 = imrotate(temp12, theta, 'bilinear', 'crop');
    temp3 = padarray(temp2, [d1,d2], 'both');
    temp4 = circshift(temp3, [round(y0(m)-d1/2),round(x0(m)-d2/2)]);
    temp = temp4((d1+1):2*d1, (d2+1):2*d2);
    temp(temp<exp(-5)) = 0;    % remove boundaries
    A(:, :, m) = temp/max(temp(:));
end
