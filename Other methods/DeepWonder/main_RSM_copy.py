...
if __name__ == '__main__':
    ...
    name_video = list_name_video[0]
    ID_part = ['_part11', '_part12', '_part21', '_part22']
    list_Exp_ID = [name_video+x for x in ID_part]
    Exp_ID = list_Exp_ID[0]
    folder = os.path.join('E:\\data_CNMFE', name_video, 'DeepWonder')

    parser = argparse.ArgumentParser()
    parser.add_argument('--GPU', type=str, default='0', help="")

    parser.add_argument('--sub_img_w', type=int, default=304, help="")
    parser.add_argument('--sub_img_h', type=int, default=304, help="") # parser.parse_args().sub_img_w
    parser.add_argument('--sub_img_s', type=int, default=1500, help="")

    parser.add_argument('--sub_gap_w', type=int, default=272, help='') # parser.parse_args().sub_img_w-32
    parser.add_argument('--sub_gap_h', type=int, default=272, help='') # parser.parse_args().sub_img_w-32
    parser.add_argument('--sub_gap_s', type=int, default=1460, help='') # parser.parse_args().sub_img_s-40

    parser.add_argument('--datasets_path', type=str, default=folder, help="")
    parser.add_argument('--datasets_folder', type=str, default=Exp_ID, help="")
    # parser.add_argument('--datasets_path', type=str, default='D:\\data_TENASPIS\\added_refined_masks\\DeepWonder', help="")
    # parser.add_argument('--datasets_folder', type=str, default='widefield', help="")
    ...
    parser.add_argument('--RMBG_model_folder', type=str, default='arr_202110011541', help="")
    ...
    parser.add_argument('--SEG_model_folder', type=str, default='TS3DUnetFFD_20211129-1355', help="")
    parser.add_argument('--SEG_model_name', type=str, default='seg_30', help='')
    ###############################################################################
    # parser.add_argument('--output_dir', type=str, default='./results', help="output directory")
    # parser.add_argument('--output_dir', type=str, default=os.path.join(parser.parse_args().datasets_path, \
    #     parser.parse_args().datasets_folder, 'DeepWonder'), help="output directory")
    parser.add_argument('--output_dir', type=str, default=os.path.join(parser.parse_args().datasets_path, \
        parser.parse_args().datasets_folder, 'DeepWonder_'+str(parser.parse_args().sub_img_w)), help="output directory")
    ...
    opt.sub_img_h = opt.sub_img_w
    opt.sub_gap_w = opt.sub_img_w-32
    opt.sub_gap_h = opt.sub_img_w-32
    ...
    for im_index in range(0, len(name_list)):
        ...
        for coor_index in range (0, len(per_coor_list)):
            ...
            # = sub_img_SEG[:num_s, patch_start_h:patch_end_h, patch_start_w:patch_end_w]
        ...
        scio.savemat(mat_save_name, {'final_mask_list':data['a'], 'final_contours':data['final_contours'],\
            'used_time':time.time() - time_start_1})
    ...
    # scio.savemat(mat_save_name, {'used_time':time_end - time_start})
