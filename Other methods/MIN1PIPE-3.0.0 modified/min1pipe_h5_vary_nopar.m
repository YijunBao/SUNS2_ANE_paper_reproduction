function [file_name_to_save, filename_raw, filename_reg] = min1pipe_h5_vary_nopar(...
    path_name, folder, filename, Fsi, Fsi_new, spatialr, se, ismc, flag,...
...
%     min1pipe_init;
    ...
    if nargin < 6 || isempty(spatialr)
        defpar = default_parameters;
        spatialr = defpar.spatialr;
    end
    
    if nargin < 7 || isempty(se)
        defpar = default_parameters;
        se = defpar.neuron_size;
    end
    
    if nargin < 8 || isempty(ismc)
        ismc = true;
    end
    ...
    if nargin < 10 || isempty(pix_select_sigthres)
        pix_select_sigthres = 0.8;
    end
    
    if nargin < 11 || isempty(pix_select_corrthres)
        pix_select_corrthres = 0.6;
    end
    
    if nargin < 12 || isempty(merge_roi_corrthres)
        merge_roi_corrthres = 0.9;
    end
    
    if nargin < 13 || isempty(dt)
        dt = 0.15;
    end
    
    if nargin < 14 || isempty(kappa)
        kappa = 0.5;
    end
    ...
    Params.anidenoise_dt = dt; % 1/7;                   %%% denoise step size %%%
    Params.anidenoise_kappa = kappa; % 0.5;       %%% denoise gradient threshold %%%
    ...
    Params.refine_roi_ispara = 0;          %%% if parallel (refine roi) %%%
...
%     [path_name, file_base, file_fmt] = data_info;
    ...
    %                 end
                ...
                roifn = zeros(pixh*pixw,0);
                process_time{2,i} = datetime;
                process_time{3,i} = datetime;
                save(file_name_to_save, 'roifn', 'pixh', 'pixw', 'Params', '-v7.3');
                save(file_name_to_save, 'imaxn', 'imeanf', 'process_time', '-append');
            end
        else
