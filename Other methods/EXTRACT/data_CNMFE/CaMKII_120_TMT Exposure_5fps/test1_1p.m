...
filenames = {'CaMKII_120_TMT Exposure_5fps_part11','CaMKII_120_TMT Exposure_5fps_part12','CaMKII_120_TMT Exposure_5fps_part21','CaMKII_120_TMT Exposure_5fps_part22'};

radii = [10:2:18];
SNRs = [15:4:31];
CV_times = zeros(length(filenames),length(radii),length(SNRs));
for p = 1:length(filenames)
    for q = 1:length(radii)
        for r = 1:length(SNRs)
            [p q r]
            output_name = ['./' filenames{p} '_radius' num2str(radii(q)) '_SNR' num2str(SNRs(r))  '.mat']
...
tic
config.avg_cell_radius=radii(q);
config.num_partitions_x=1;
config.num_partitions_y=1;

% change these as needed
config.cellfind_min_snr = 1;
config.thresholds.T_min_snr=SNRs(r);
