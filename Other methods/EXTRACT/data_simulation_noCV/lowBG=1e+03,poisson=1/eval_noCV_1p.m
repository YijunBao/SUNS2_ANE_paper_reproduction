...
for n = 1:(length(filenames)*length(radii)*length(SNRs))
...
load(fullfile(output_folder,'noCV_times.mat'))
save(fullfile(output_folder,'eval_noCV.mat'), 'list_Recall', 'list_Precision', ...
    'list_F1', 'indq', 'indr', 'CV_times', 'radii', 'SNRs');

Params_all=[indq*radii, indr*SNRs, list_Recall, list_Precision, list_F1, CV_times]; % 
Params_all_ext=[Params_all;nanmean(Params_all,1);nanstd(Params_all,1,1)]; %([1,2,4:10],:)
disp(Params_all_ext(end-1,:))
