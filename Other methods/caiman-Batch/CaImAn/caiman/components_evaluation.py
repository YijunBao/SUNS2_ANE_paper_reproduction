...
def evaluate_components_CNN(A, dims, gSig, model_name:str=os.path.join(caiman_datadir(), 'model', 'cnn_model'), patch_size:int=50, loaded_model=None, isGPU:bool=True) -> Tuple[Any, np.array]:
...
def estimate_components_quality_auto(Y, A, C, b, f, YrA, frate, decay_time, gSig, dims, dview=None, 
                                     model_name=os.path.join(caiman_datadir(), 'model', 'cnn_model'), min_SNR=2, r_values_min=0.9,
    ...
    idx_components, idx_components_bad, cnn_values = select_components_from_metrics(
                ...
                thresh_cnn_min, thresh_cnn_lowest, use_cnn, gSig_range,model_name)
    ...
    if use_cnn:
        ...
        if gSig_range is None:
            if predictions is None:
                predictions, _ = evaluate_components_CNN(A, dims, gSig,model_name=model_name)
            ...
            for size_range in gSig_range:
                predictions = np.maximum(predictions,
                                         evaluate_components_CNN(A, dims, size_range,model_name=model_name)[0][:, neuron_class])
