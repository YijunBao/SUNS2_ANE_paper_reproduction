...
#                                 Loading Object From File")
    ...
    def _prepare_object(self, Yr, T, new_dims=None, idx_components=None):
        ...
        # self.estimates.CY.resize([expected_comps + self.params.get('init', 'nb'), self.estimates.CY.shape[-1]]) # Replaced by one of the following two lines. --YB 2019/12/11
        self.estimates.CY.resize([expected_comps + self.params.get('init', 'nb'), self.estimates.CY.shape[-1]], refcheck=False)
        # self.estimates.CY = np.resize(self.estimates.CY, (expected_comps + self.params.get('init', 'nb'), self.estimates.CY.shape[-1]))
            ...
            if num_added > 0:
                ...
                if self.N + self.params.get('online', 'max_num_added') > expected_comps:
                    ...
                    print('Increasing number of expected components to:' +
                          str(expected_comps))
                self.update_counter.resize(self.N, refcheck=False) # Added refcheck by YB
