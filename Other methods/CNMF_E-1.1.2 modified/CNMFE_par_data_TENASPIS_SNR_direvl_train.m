...
data_name = 'TENASPIS_SNR';
% path_name = '../data/data_TENASPIS/added_refined_masks';
path_name = 'D:\data_TENASPIS\added_refined_masks\complete_TUnCaT_SF25\network_input';
...
load_date = '20221221';
load(fullfile(dir_save,['eval_',data_name(1:end-4),'_thb history ',load_date,'.mat']),'list_seq');

%% pre-load the data to memory
for eid = 1:num_Exp
    Exp_ID = list_Exp_ID{eid};
    video = h5read(fullfile(path_name,[Exp_ID,'.h5']),'/network_input');
    clear video;
end
...
init_ind_param = [1, 3, 2, 7, 5, 6, 8, 4]'; % SNR
...
%                             Masks3 = neuron.reshape(Ab, 2); 
                ...
                %         load(fullfile(dir_save,dir_sub,[Exp_ID,'_Masks.mat']),'Masks3');
                        ...
                        for tid = 1:num_thb
                            thb = list_th_binary(tid);
%                             saved_Masks3 = fullfile(dir_save,dir_sub,[Exp_ID,'_Masks_',num2str(thb),'.mat']);
%                             if exist(saved_Masks3,'file')
%                                 Masks3 = load(saved_Masks3,'Masks3');
%                                 Masks3 = Masks3.Masks3;
%                             else
                            Masks3 = threshold_Masks(A3, thb); %;%
%                             end
%                             Ab = A>thb*max(A,[],1); %;%
...
%                     Table_time = [best_Recall(:,ind_thb), best_Precision(:,ind_thb), best_F1(:,ind_thb), best_time(:,ind_thb)];
                ...
                save(fullfile(dir_save,['eval_',data_name,'_thb history.mat']),'list_seq',...
...
%%
movefile(fullfile(dir_save,['eval_',data_name,'_thb history.mat']), ...
    fullfile(dir_save,['eval_',data_name,'_thb history ',num2str(yyyymmdd(datetime)),'.mat']));
