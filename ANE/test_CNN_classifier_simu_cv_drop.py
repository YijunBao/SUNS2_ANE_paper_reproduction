...
if __name__ == "__main__":
    # sys.argv = ['.py', 'classifier_res0', '0', 'Xmask', '0', '1', '_weighted_sum_unmask', '0.8exp(-10)', 'lowBG=1e+03,poisson=1']
    ...
    name_video = sys.argv[8] # 'lowBG=5e+03,poisson=0.3'
    dir_parent = os.path.join('E:\\simulation_CNMFE_corr_noise', name_video)
    num_Exp = 10
    ...
    for opt_th_cnn in list_opt_th_cnn:
        for version in [0]: # range(2,9): # range(0,10): # 
            ...
            dir_CNN = os.path.join(dir_parent, 'GT Masks dropout {}\\add_new_blockwise{}\\classifiers_{}'.format(drop_rate, folder_sub, sub), folder)

            # model = CNN_classifier(input_shape, num_classes)
            for th_SNR in [4]: # range(3,6):
                list_dir_test = [os.path.join(dir_parent, 'complete_FISSA\\4816[1]th{}\\output_masks\\add_new_blockwise{}'.format(2, folder_sub))]
                # list_dir_test = [os.path.join(dir_parent, 'complete_TUnCaT\\4816[1]th{}\\output_masks\\add_new_blockwise{}'.format(th_SNR, folder_sub))]
                for dir_test in list_dir_test:
                    ...
                    if opt_th_cnn:
                        dir_save = os.path.join(dir_SUNS_out, sub, folder) # 
                    else:
                        dir_save = os.path.join(dir_SUNS_out, sub+'_0.5', folder) # 
