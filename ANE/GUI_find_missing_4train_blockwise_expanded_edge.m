function GUI_find_missing_4train_blockwise_expanded_edge(video, folder, masks, patch_locations, ...
...
%     [Lxm, Lym, ~] = size(video_sub_sort);
    ...
    [~, ~, current_thred] = threshold_frame(avg_frame, thred_inten, avg_area);
...
function LowerThreshold(~,~)
    ...
    [mask_update, ~, current_thred] = threshold_frame(avg_frame_use, current_thred*0.9);
    delete(new_contour);
    [~, new_contour] = contour(gui.AvgAxes,mask_update,'r');
end
...
function HigherThreshold(~,~)
    avg_frame_use = avg_frame;
    [mask_update, ~, current_thred] = threshold_frame(avg_frame_use, current_thred*1.1);
    delete(new_contour);
    [~, new_contour] = contour(gui.AvgAxes,mask_update,'r');
end
...
%     list_avg_frame{nn} = avg_frame;
    ...
    thred_inten = quantile(avg_frame, q, 'all');
    [mask_update, ~, current_thred] = threshold_frame(avg_frame, thred_inten, avg_area);
