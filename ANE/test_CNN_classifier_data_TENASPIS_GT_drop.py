...
if __name__ == "__main__":
    ...
    for opt_th_cnn in list_opt_th_cnn:
        for version in [0]: # range(2,9): # range(0,10): # 
            ...
            # dir_CNN = os.path.join('D:\\data_TENASPIS\\original_masks\\GT Masks dropout {}\\add_new_blockwise{}\\classifiers_{}'.format(drop_rate, folder_sub, sub), folder)
            dir_CNN = os.path.join('D:\\data_TENASPIS\\added_refined_masks\\GT Masks dropout {}\\add_new_blockwise{}\\classifiers_{}'.format(drop_rate, folder_sub, sub), folder)

            # model = CNN_classifier(input_shape, num_classes)
            for th_SNR in [5]: # range(3,6):
                list_dir_test = []
                # list_dir_test.append('D:\\data_TENASPIS\\original_masks\\GT Masks dropout {}\\add_new_blockwise{}'.format(drop_rate, folder_sub))
                list_dir_test.append('D:\\data_TENASPIS\\added_refined_masks\\GT Masks dropout {}\\add_new_blockwise{}'.format(drop_rate, folder_sub))
