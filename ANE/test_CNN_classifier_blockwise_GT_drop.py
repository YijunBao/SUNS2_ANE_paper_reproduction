...
if __name__ == "__main__":
    ...
    list_Exp_ID = [name_video+x for x in ID_part] # +'_added'
    ...
    for opt_th_cnn in list_opt_th_cnn:
        for version in [0]: # range(2,9): # range(0,10): # 
            ...
            drop_rate = sys.argv[7] # 0.8exp(-10)
            ...
            dir_CNN = os.path.join('E:\\data_CNMFE\\{}\\GT Masks dropout {}\\add_new_blockwise{}\\classifiers_{}'.format(name_video, drop_rate, folder_sub, sub), folder)
            # dir_CNN = os.path.join('E:\\data_CNMFE\\{}_added_blockwise_weighted_sum_unmask\\GT Masks dropout {}\\add_new_blockwise{}\\classifiers_{}'.format(name_video, drop_rate, folder_sub, sub), folder)

            # model = CNN_classifier(input_shape, num_classes)
            for th_SNR in [4]: # range(3,6):
                list_dir_test = []
                list_dir_test.append('E:\\data_CNMFE\\{}\\GT Masks dropout {}\\add_new_blockwise{}'.format(name_video, drop_rate, folder_sub))
                # list_dir_test.append('E:\\data_CNMFE\\{}_added_blockwise_weighted_sum_unmask\\GT Masks dropout {}\\add_new_blockwise{}'.format(name_video, drop_rate, folder_sub))
                for dir_test in list_dir_test:
                    ...
                    for (cv,Exp_ID) in enumerate(list_Exp_ID): #
                        ...
                        if num_mask_channel == 1:
                            ...
                            added_auto = sio.loadmat(os.path.join(dir_test, Exp_ID+'_added_CNNtrain_blockwise.mat'))
