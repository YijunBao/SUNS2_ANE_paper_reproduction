...
if __name__ == "__main__":
    # sys.argv = ['.py', 'classifier_res0', '2', 'mask', '0', '2']
    ...
    for opt_th_cnn in list_opt_th_cnn:
        for version in [0]: # range(2,9): # range(0,10): # 
            ...
            dir_CNN = os.path.join('E:\\data_CNMFE\\full videos\\GT Masks\\add_new_blockwise\\classifiers_' + sub, folder)

            # model = CNN_classifier(input_shape, num_classes)
            training_output = sio.loadmat(os.path.join(dir_CNN, "training_output_{}.mat".format(name_video)))
            if opt_th_cnn:
                th_cnn = training_output['th_cnn_best']
            else:
                th_cnn = 0.5
            model_name = os.path.join(dir_CNN, 'cnn_model_'+name_video)
            ...
            for th_SNR in range(3,6):
                list_dir_test = []
                # list_dir_test.append('E:\\data_CNMFE\\{}_original_masks\\complete_TUnCaT\\4816[1]th{}\\output_masks\\add_new_blockwise'.format(name_video,th_SNR))
                list_dir_test.append('E:\\data_CNMFE\\{}_added_auto\\complete_TUnCaT\\4816[1]th{}\\output_masks\\add_new_blockwise'.format(name_video,th_SNR))
                for dir_test in list_dir_test:
                    ...
                    for Exp_ID in list_Exp_ID: #
                        ...
                        sio.savemat(os.path.join(dir_save,"CNN_predict_{}.mat".format(Exp_ID)), 
