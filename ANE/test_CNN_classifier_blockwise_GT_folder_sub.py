...
if __name__ == "__main__":
    ...
    for opt_th_cnn in list_opt_th_cnn:
        for version in [0]: # range(2,9): # range(0,10): # 
            ...
            dir_CNN = os.path.join('E:\\data_CNMFE\\{}_original_masks\\GT Masks\\add_new_blockwise{}\\classifiers_{}'.format(name_video, folder_sub, sub), folder)
            # dir_CNN = os.path.join('E:\\data_CNMFE\\{}_added_blockwise_weighted_sum_unmask\\GT Masks\\add_new_blockwise{}\\classifiers_{}'.format(name_video, folder_sub, sub), folder)

            # model = CNN_classifier(input_shape, num_classes)
            for th_SNR in [4]: # range(3,6):
                list_dir_test = []
                list_dir_test.append('E:\\data_CNMFE\\{}_original_masks\\GT Masks\\add_new_blockwise{}'.format(name_video, folder_sub))
                # list_dir_test.append('E:\\data_CNMFE\\{}_added_blockwise_weighted_sum_unmask\\GT Masks\\add_new_blockwise{}'.format(name_video, folder_sub))
