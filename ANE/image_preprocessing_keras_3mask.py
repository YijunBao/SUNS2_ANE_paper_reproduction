...
def shuffle_channel(x, channel_axis=0):
    x = np.rollaxis(x, channel_axis, 0)
...
def random_mutiplication(x, range_mult, img_channel_axis):
    x = x * np.random.uniform(range_mult[0], range_mult[1])
    # x = np.asarray(x).swapaxes(img_channel_axis, 0)
    # x[0] = x[0] * np.random.uniform(range_mult[0], range_mult[1])
    # x = x.swapaxes(0, img_channel_axis)
    return x
...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        self.random_shuffle_channel = random_shuffle_channel
        self.binary_last_channel = binary_last_channel
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.random_mult_range != 0:
            if np.random.random() < 0.5:
                x = random_mutiplication(x, self.random_mult_range, img_channel_axis)

        if self.random_shuffle_channel:
            x = shuffle_channel(x, img_channel_axis)

        if self.binary_last_channel:
            x = np.asarray(x).swapaxes(img_channel_axis, 0)
            for c in range(1,self.binary_last_channel+1):
                x[-c] = x[-c] > (x[-c].min()+x[-c].max())/2
            x = x.swapaxes(0, img_channel_axis)
