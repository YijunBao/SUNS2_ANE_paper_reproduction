...
% list_d=[5,6]; % bma22_epm
% list_d=[10,12]; % [8,10]; % CaMKII_120_TMT Exposure_5fps
% list_d=[4:10]; % two element array showing the minimum and maximum allowed SNR
...
for vid=1
    ...
    dir_masks = fullfile(dir_parent,[Exp_ID],'GT Masks'); % ,'_original_masks'
    dir_trace = fullfile(dir_parent,[Exp_ID],'traces'); % ,'_original_masks'
    fs = rate_hz(vid);
    traces_in = cell(4,1);
    ROIs = cell(1,1,4);
    part = 0;
    for xpart = 1:2
        for ypart = 1:2
            ...
            load(fullfile(dir_trace,['raw and bg traces ',Exp_ID_part,'.mat']),'traces_raw','traces_bg_exclude');
