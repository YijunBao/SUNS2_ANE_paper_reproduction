...
neuron_amp = 0.003; 
dir_GT_info = fullfile(dir_parent, ['add_neurons_',num2str(neuron_amp),'_rotate'],'GT info');
...
%     dir_GT_info = fullfile(dir_video, 'GT info'); % , sub_folder
    ...
    for mcid=1:length(list_num_mask_channel)
        ...
        sub0 = sprintf('trained addfake_%g_rotate',neuron_amp);
...
%                 Masks=permute(logical(Masks),[3,2,1]);
        ...
        Table_all(vid,(did-1)*5+(1:5)) = nanmean(Table_ext(end-1,1:5),1);
...
%         disp('Finished this step');
        %%
        Table = [list_Recall_add_max,list_Recall_max,list_Precision_max,list_F1_max,list_time_SUNS];
        Table_ext = [Table;nanmean(Table,1);nanstd(Table,1,1)];    
        Table_max(1,(did-1)*5+(1:5)) = nanmean(Table_ext(end-1,1:5),1);
    end
    end
end
