...
for neuron_amp = [0.005, 0.001, 0.002, 0.003]
data_ind = 2;
...
dir_add_new = fullfile(dir_video, 'add_new_blockwise_weighted_sum_unmask');
fs = rate_hz(data_ind);
...
for eid = 1:num_Exp
    ...
    fname=fullfile(dir_video,[Exp_ID,'_added.h5']);
...
%     load(fname, 'video_SNR');
    ...
    traces_raw=generate_traces_from_masks(video_SNR,masks);
...
%     load(fullfile(dir_trace,['raw and bg traces ',Exp_ID,'.mat']),'traces_raw','traces_bg_exclude','traces_out_exclude');
    ...
    [list_weight,list_weight_trace,list_weight_frame,sum_edges]...
        = frame_weight_blockwise(video, traces_raw, masks, leng);
...
%     masks_sum = sum(masks,3);
    ...
    for iy = 1:npatchy % npatchy % 
        ...
        [image_new_crop, mask_new_crop, mask_new_full, select_frames_class, select_weight_calss] = ...
            find_missing_blockwise_weighted_sum_unmask(video, masks, xmin, xmax, ymin, ymax, ...
