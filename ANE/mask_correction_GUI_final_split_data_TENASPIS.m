...
%%
nn = 482;
filebase = fullfile(dir_parent,'refine_from_added',Exp_ID,'masks_update(1--615)-3+8');
filename0 = [filebase,'.mat'];
filename1 = [filebase,'+.mat'];
filename2 = [filebase,'++.mat'];

%%
load(filename1,'update_result');
update_result1 = update_result;
load(filename2,'update_result');
update_result2 = update_result;
load(filename0,'update_result');
...
%%
list_mask_update{nn} = update_result1.list_mask_update{nn};
masks_update(:,:,nn) = update_result1.masks_update(:,:,nn);
ListStrings{nn} = [num2str(nn),' Added'];
list_added{end+1} = update_result2.masks_update(:,:,nn);
