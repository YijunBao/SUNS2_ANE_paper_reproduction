function [image_new_crop, mask_new_crop, mask_new_full, select_frames_class, select_weight_calss] = ...
    find_missing_blockwise_expanded_edge(video, masks, xmin, xmax, ymin, ymax, ...
...
%     frame(~nearby_disk) = 0;
    ...
    [frame_thred, noisy] = threshold_frame(frame, thred_inten, avg_area);
    if ~noisy
...
%     avg_frame_use(~nearby_disk) = 0;
    ...
    mask_update{c} = threshold_frame(avg_frame_use, thred_inten, avg_area);
end
...
% if xmin == 1
%     pad_fun = @(x) padarray(x,[Leng-Lxm,0],0,'pre');
%     select_frames_class = cellfun(pad_fun, select_frames_class, 'UniformOutput',false);
%     mask_new_crop = pad_fun(mask_new_crop);
%     image_new_crop = pad_fun(image_new_crop);
% elseif xmax == Lx
%     pad_fun = @(x) padarray(x,[Leng-Lxm,0],0,'post');
%     select_frames_class = cellfun(pad_fun, select_frames_class, 'UniformOutput',false);
%     mask_new_crop = pad_fun(mask_new_crop);
%     image_new_crop = pad_fun(image_new_crop);
% end
% if ymin == 1
%     pad_fun = @(x) padarray(x,[0,Leng-Lym],0,'pre');
%     select_frames_class = cellfun(pad_fun, select_frames_class, 'UniformOutput',false);
%     mask_new_crop = pad_fun(mask_new_crop);
%     image_new_crop = pad_fun(image_new_crop);
% elseif ymax == Ly
%     pad_fun = @(x) padarray(x,[0,Leng-Lym],0,'post');
%     select_frames_class = cellfun(pad_fun, select_frames_class, 'UniformOutput',false);
%     mask_new_crop = pad_fun(mask_new_crop);
%     image_new_crop = pad_fun(image_new_crop);
% end
