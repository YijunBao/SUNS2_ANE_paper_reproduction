...
if __name__ == "__main__":
    ...
    # dir_video = 'E:\\data_CNMFE\\full videos\\classifier'
    dir_video = os.path.join('E:\\data_CNMFE\\full videos\\GT Masks\\add_new_blockwise')
    ...
    added_auto = sio.loadmat(os.path.join(dir_video, name_video+'_added_CNNtrain_blockwise.mat'))
    ...
    # x_train = [np.stack([added_frames[i], mask_train[:,:,i]], axis=0) for i in range(y_train.size)]
    # x_train = np.stack([image_train, mask_train], axis=0)
    # x_train = np.stack([image_train, mask_train*image_train], axis=0)
    ...
    model_name = 'cnn_model_'+name_video #str(datetime.datetime.now()).replace(' ', '-').replace(':', '-')
    ...
    sio.savemat(os.path.join(save_dir,"training_output_{}.mat".format(name_video)), history)
