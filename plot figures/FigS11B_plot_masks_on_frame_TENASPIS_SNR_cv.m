...
data_name = 'TENASPIS';
% dir_video = fullfile('../data/data_TENASPIS/added_refined_masks/SUNS_TUnCaT_SF25/network_input');
dir_video = fullfile('D:/data_TENASPIS/added_refined_masks/complete_TUnCaT_SF25/network_input');
dir_video_raw = dir_video;
% varname = '/mov';
dir_video_SNR = fullfile(dir_video,'../network_input');
dir_traces = fullfile(dir_video,'../TUnCaT/alpha= 1.000');
dir_GT_masks = fullfile(dir_video,'GT Masks');
dir_MIN1PIPE = fullfile(dir_video,'min1pipe');
dir_CNMFE = fullfile(dir_video,'CNMFE');
dir_sub_min1pipe = 'cv_save_20240226';
dir_sub_CNMFE = 'cv_save_20240321';
% saved_date_MIN1PIPE = '20240226 test';
% saved_date_CNMFE = '20240321 test';
mag=1;
mag_crop=4;
%%
for k=3%:num_Exp_ID
...
save_folder = sprintf('figures_SNR_%d-%d,%d-%d crop',xrange(1),xrange(end),yrange(1),yrange(end));
...
%% SUNS TUnCaT
dir_output_mask = fullfile(dir_video,'../../complete_TUnCaT_noSF/4816[1]th2/output_masks');
...
for n = 1:N_SUNS
    temp = Masks_SUNS_mag(xrange_mag,yrange_mag,n);
    if any(temp,'all')
        contour(temp, 'EdgeColor',colors(7,:),'LineWidth',0.5);
    end
end

if ~image_only
    title(sprintf('%s, SUNS TUnCaT noSF, F1 = %1.2f',Exp_ID,F1),'FontSize',12,'Interpreter','None');
    h=colorbar;
    set(get(h,'Label'),'String','Peak SNR');
    set(h,'FontSize',12);
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks SUNS_TUnCaT noSF',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks SUNS_TUnCaT noSF',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
    % saveas(gcf,['figure 2/',Exp_ID,' SUNS noSF h.svg']);
end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks SUNS_TUnCaT noSF ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks SUNS_TUnCaT noSF ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks SUNS_TUnCaT noSF ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
%     imwrite(cdata,[save_folder, data_name,' Masks SUNS_TUnCaT noSF ',list_title{k},'.tif']);
end


%% SUNS FISSA
dir_output_mask = fullfile(dir_video,'../../complete_FISSA_noSF/4816[1]th2/output_masks');
load(fullfile(dir_output_mask, ['Output_Masks_', Exp_ID, '.mat']), 'Masks');
Masks_SUNS = permute(Masks,[3,2,1]); % FinalMasks; % 
N_SUNS = size(Masks_SUNS,3);
if mag > 1
...
for n = 1:N_SUNS
    temp = Masks_SUNS_mag(xrange_mag,yrange_mag,n);
    if any(temp,'all')
        contour(temp, 'EdgeColor',colors(8,:),'LineWidth',0.5);
    end
end

if ~image_only
    title(sprintf('%s, SUNS FISSA noSF, F1 = %1.2f',Exp_ID,F1),'FontSize',12,'Interpreter','None');
    h=colorbar;
    set(get(h,'Label'),'String','Peak SNR');
    set(h,'FontSize',12);
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks SUNS_FISSA noSF ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks SUNS_FISSA noSF ',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
    % saveas(gcf,['figure 2/',Exp_ID,' SUNS noSF h.svg']);
end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks SUNS_FISSA noSF ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks SUNS_FISSA noSF ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks SUNS_FISSA noSF ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
%     imwrite(cdata,[save_folder, data_name,' Masks SUNS_FISSA noSF ',list_title{k},'.tif']);
end
...
for n = 1:N_min1
    ...
    if any(temp,'all')
        contour(temp, 'EdgeColor',(1+color(1,:))/2,'LineWidth',0.5);
    end
end

if ~image_only
    title(sprintf('%s, MIN1PIPE SNR, F1 = %1.2f',Exp_ID,F1),'FontSize',12,'Interpreter','None');
    h=colorbar;
    set(get(h,'Label'),'String','Peak SNR');
    set(h,'FontSize',12);
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks min1pipe SNR ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks min1pipe SNR ',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks min1pipe SNR ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks min1pipe SNR ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks min1pipe SNR ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
%     imwrite(cdata,[save_folder, data_name,' Masks min1pipe SNR ',list_title{k},'.tif']);
end
...
for n = 1:N_cnmfe
    ...
    if any(temp,'all')
        contour(temp, 'EdgeColor',(0.5+color(2,:))/1.5,'LineWidth',0.5);
    end
end

if ~image_only
    title(sprintf('%s, CNMF-E SNR, F1 = %1.2f',Exp_ID,F1),'FontSize',12,'Interpreter','None');
    h=colorbar;
    set(get(h,'Label'),'String','Peak SNR');
    set(h,'FontSize',12);
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks CNMF-E SNR ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks CNMF-E SNR ',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
    % saveas(gcf,['figure 2/',Exp_ID,' CaImAn Batch h.svg']);
end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks CNMF-E SNR ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks CNMF-E SNR ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks CNMF-E SNR ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
%     imwrite(cdata,[save_folder, data_name,' Masks CNMF-E SNR ',list_title{k},'.tif']);
end


%% CNMF (CaImAn)
dir_output_mask = fullfile(dir_video,'../../caiman-Batch\Masks');
load(fullfile(dir_output_mask, [Exp_ID, '_neurons.mat']), 'finalSegments');
Masks_CNMF = finalSegments;
N_CNMF = size(Masks_CNMF,3);

if mag > 1
    Masks_CNMF_mag=zeros(Lxm,Lym,N_CNMF,'logical');
    for n = 1:N_CNMF
        Masks_CNMF_mag(:,:,n) = kron(Masks_CNMF(:,:,n),mag_kernel_bool);
    end
else
    Masks_CNMF_mag = Masks_CNMF;
end

[Recall, Precision, F1, m] = GetPerformance_Jaccard(dir_GT_masks,Exp_ID,Masks_CNMF,0.5);

TP_6 = sum(m,1)>0;
TP_62 = sum(m,2)>0;
FP_6 = sum(m,1)==0;
FN_6 = sum(m,2)==0;
masks_TP = sum(Masks_CNMF(:,:,TP_6),3);
masks_FP = sum(Masks_CNMF(:,:,FP_6),3);
...
for n = 1:N_CNMF
    temp = Masks_CNMF_mag(xrange_mag,yrange_mag,n);
    if any(temp,'all')
        contour(temp, 'EdgeColor',colors_multi(28,:),'LineWidth',0.5);
    end
end

if ~image_only
    title(sprintf('%s, CNMF, F1 = %1.2f',Exp_ID,F1),'FontSize',12,'Interpreter','None');
    h=colorbar;
    set(get(h,'Label'),'String','Peak SNR');
    set(h,'FontSize',12);
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks CNMF ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks CNMF ',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
    % saveas(gcf,['figure 2/',Exp_ID,' CaImAn Batch h.svg']);
end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks CNMF ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks CNMF ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks CNMF ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
...
%% EXTRACT
% dir_output_mask = fullfile(dir_video,'CNMF',Exp_ID,'CNMF/mat');
% load(fullfile(dir_output_mask, ['seg_30_',Exp_ID, '_post.mat']), 'network_A_filt');
dir_output_mask = fullfile(dir_video,'EXTRACT/EXTRACT_CV');
...
for n = 1:N_EXTRACT
    ...
    if any(temp,'all')
        contour(temp, 'EdgeColor',colors_multi(21,:)/2,'LineWidth',0.5);
    end
end
...
if save_figures
    if image_only
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks EXTRACT SNR ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    else
        saveas(gcf,fullfile(save_folder, [data_name, ' Masks EXTRACT SNR ',list_title{k},' ',mat2str(SNR_range),'.png']));
    end
    % saveas(gcf,['figure 2/',Exp_ID,' CaImAn Batch h.svg']);
end
...
if save_figures
    imwrite(zoom1_mag,fullfile(save_folder,[data_name,' Masks EXTRACT SNR ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[data_name,' Masks EXTRACT SNR ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[data_name,' Masks EXTRACT SNR ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
%     imwrite(cdata,[save_folder, data_name,' Masks EXTRACT SNR ',list_title{k},'.tif']);
end
