...
PSNR = max(trace_N);
high = find(trace_N > min(10,PSNR-0.8));
...
PSNR = max(trace_N);
high = find(trace_N > min(10,PSNR-1.5));
% start = [rect1(2)+xrange(1)-1, rect1(1)+yrange(1)-1, 1];
...
%% SUNS + missing finder
dir_output_mask_SUNS = fullfile(dir_video,'complete_TUnCaT_SF25\4816[1]th6\output_masks');
dir_output_mask = fullfile(dir_output_mask_SUNS,'add_new_blockwise\trained dropout 0.8exp(-15)');
...
rectangle('Position',[140,17,16,4],'FaceColor','w','LineStyle','None'); % 20 um scale bar
...
%% SUNS TUnCaT
dir_output_mask = fullfile(dir_video,'complete_TUnCaT_SF25\4816[1]th6\output_masks');
% load([dir_output_mask, Exp_ID, '.mat'], 'Masks_2');
% Masks = reshape(full(Masks_2'),487,487,[]);
...
% figure(98)
% subplot(2,3,4)
figure('Position',[1750,50,600,500],'Color','w');
%     imshow(raw_max,[0,1024]);
if image_only
    imshow(SNR_max_mag(xrange_mag,yrange_mag),SNR_range);
else
...
if save_figures
    if image_only
        saveas(gcf,[save_folder, data_name, ' Masks DeepWonder ',list_title{k},' ',mat2str(SNR_range),'.tif']);
    else
        saveas(gcf,[save_folder, data_name, ' Masks DeepWonder ',list_title{k},' ',mat2str(SNR_range),'.png']);
    end
    % saveas(gcf,['figure 2\',Exp_ID,' CaImAn Batch h.svg']);
end
...
better = (TP_12 & ~TP_22 & ~TP_32 & ~TP_42 & ~TP_52); %  & ~TP_42 & ~TP_52
