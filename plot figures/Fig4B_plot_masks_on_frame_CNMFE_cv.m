...
switch data_ind 
    case 1
        save_date_MIN1PIPE = '20221216';
        save_date_CNMFE = '20221216';
    case 2
        save_date_MIN1PIPE = '20221216';
        save_date_CNMFE = '20221215';
    case 3
        save_date_MIN1PIPE = '20221219';
        save_date_CNMFE = '20221218';
    case 4
        save_date_MIN1PIPE = '20221215';
        save_date_CNMFE = '20221215';
end
...
% rect2=[74,36,60,60];
% rect3=[134,31,55,90];
rect1_mag = rect1 * mag - (mag-1)*[1,1,0,0];
rect2_mag = rect2 * mag - (mag-1)*[1,1,0,0];
rect3_mag = rect3 * mag - (mag-1)*[1,1,0,0];
rect1_sub=rect1_mag - [yrange(1)-1,xrange(1)-1,0,0];
rect2_sub=rect2_mag - [yrange(1)-1,xrange(1)-1,0,0];
rect3_sub=rect3_mag - [yrange(1)-1,xrange(1)-1,0,0];
SNR_range = [0,3]; % [2,14]; % 
scale_bar = [5,110,20,6]*mag;
...
pos1=1200;
pos2=2;
plot(pos1+[0,500],pos2*[1,1],'k','LineWidth',2);
...
%% SUNS + ANE
dir_output_mask_SUNS = fullfile(dir_video,'SUNS_TUnCaT_SF25/4816[1]th5/output_masks');
...
if save_figures
    ...
    imwrite(zoom1_mag,fullfile(save_folder,[' Masks SUNS_ANE ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[' Masks SUNS_ANE ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[' Masks SUNS_ANE ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
end


%% SUNS TUnCaT
dir_output_mask = fullfile(dir_video,'SUNS_TUnCaT_SF25/4816[1]th5/output_masks');
...
if save_figures
    imwrite(cdata,fullfile(save_folder,[' Masks EXTRACT ',...
        list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom1_mag,fullfile(save_folder,[' Masks EXTRACT ',...
        num2str(N_neuron1),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom2_mag,fullfile(save_folder,[' Masks EXTRACT ',...
        num2str(N_neuron2),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
    imwrite(zoom3_mag,fullfile(save_folder,[' Masks EXTRACT ',...
        num2str(N_neuron3),' ',list_title{k},' ',mat2str(SNR_range),'.tif']));
end
