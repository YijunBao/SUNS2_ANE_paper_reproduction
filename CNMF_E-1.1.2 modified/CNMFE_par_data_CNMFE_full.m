...
list_params.rbg = 2; % [1.5, 1.8, 2]; % 
list_params.nk = 3; % [1, 2, 3]; % 
list_params.rdmin = 2.5; % [2, 2.5, 3, 4]; % 
list_params.min_corr = 0.98; % [0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 0.98];
list_params.min_pnr = 8; % 4:4:24;
list_params.merge_thr = 0.7; % 0.1:0.1:0.9;
list_params.merge_thr_spatial = 0.6; % 0.1:0.1:0.9;
list_params.merge_thr_temporal = 0.1; % 0.1:0.1:0.9;
...
%                     Table_time = [best_Recall(:,ind_thb), best_Precision(:,ind_thb), best_F1(:,ind_thb), best_time(:,ind_thb)];
                ...
                save(['eval_',Exp_ID,'_thb full.mat'],'list_seq',...
...
%%
movefile(['eval_',Exp_ID,'_thb full.mat'], ['eval_',Exp_ID,'_thb full ',num2str(yyyymmdd(datetime)),'.mat']);
...
%% Extract background from final result
neuron_final = neuron;
output_bg_info = neuron_final.update_background_parallel_save(1);
% save(fullfile(dir_save,['output_bg_info_',Exp_ID]),'output_bg_info','-v7.3');
