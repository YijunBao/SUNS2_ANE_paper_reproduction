...
% list_min_pnr = 8;
% list_merge_thr = 0.65;
% list_merge_thr_spatial = 0.8;
% list_merge_thr_temporal = 0.4;
...
% list_gSiz = 2 * radius(data_ind); % 7; %[6,7,8,9]; % 
% list_nk = 3; % [1,3];
% list_rdmin = 3; % [2,3,4]; % 
% list_min_corr = [0.9, 0.95]; % [0.6, 0.8, 0.9]; % 0.9, 
% list_min_pnr = 16; % [8, 12, 16]; % [4,8,12]; % 
% list_merge_thr = 0.6; % [0.4, 0.6, 0.8]; % 
% list_merge_thr_spatial = [0.4, 0.6, 0.8]; % 0.6; % 
% list_merge_thr_temporal = [0.1, 0.2, 0.4]; % 0.2; % 

% list_nk = 3; % [1,3]; % 
% list_gSiz = 12; %[6,7,8,9]; % 
% list_rdmin = [2,2.5,3]; % 3; % 
% list_min_corr = [0.8, 0.9, 0.95, 0.98]; % 0.9; % , 0.95
% list_min_pnr = [8,12,16]; % 8; % 
% list_merge_thr = 0.65; % [0.4, 0.6, 0.8]; % 
...
% list_nk = 3; % [1,3];
% list_rdmin = [2,3,4]; % 3; % 
% list_min_corr = [0.9, 0.95]; % [0.6, 0.8, 0.9]; % 
% list_min_pnr = [8, 12, 16]; % 8; % [4,8,12]; % 
% list_merge_thr = 0.6; % [0.4, 0.6, 0.8]; % 
% list_merge_thr_spatial = 0.4; % [0.2, 0.4, 0.6]; % [0.6, 0.8, 0.9]; %  
% list_merge_thr_temporal = [0.1, 0.2]; % 0.2; % , 0.4, 0.6
...
%         rdmin,min_corr,min_pnr,merge_thr,mts,mtt);
    dir_sub = sprintf('gSiz=%d,rbg=%0.1f,nk=%d,rdmin=%0.1f,mc=%0.2f,mp=%d,mt=%0.2f,mts=%0.2f,mtt=%0.2f',...
        list_gSiz,1.5,list_nk,rdmin,min_corr,min_pnr,merge_thr,mts,mtt);
    if ~ exist(fullfile(dir_save,dir_sub),'dir')
        mkdir(fullfile(dir_save,dir_sub));
    end
    
    if ~exist(fullfile(dir_save,dir_sub,[Exp_ID,'_Masks.mat']),'file')
        %% parameters
        % -------------------------    COMPUTATION    -------------------------  %
...
%         saved_files = dir(fullfile(child,'frames*\LOGS*\*May*.mat'));
...
%         movefile(fullfile(saved_file.folder,saved_file.name), fullfile(dir_save,dir_sub,[Exp_ID,'_result.mat']));
        ...
        save(fullfile(dir_save,dir_sub,[Exp_ID,'_Masks.mat']),'Masks3');
...
%         load(fullfile(dir_save,dir_sub,[Exp_ID,'_Masks.mat']),'Masks3');
    ...
    %% save neurons shapes
    % neuron.save_neurons();
    save(['eval_',data_name,'.mat'],'list_Recall','list_Precision','list_F1','list_time','list_gSiz','list_rdmin',...
