...
for cv = 6:num_Exp
    ...
    load(['.\eval_',data_name,'_thb history ',save_date,' cv', num2str(cv),'.mat'],...
        'list_params','history','best_history','list_seq','best_ind_param',...
        'best_F1','best_Recall','best_Precision','best_time','best_thb');
...
%                                 Masks3 = neuron.reshape(Ab, 2); 
    ...
    movefile(['eval_',data_name,'_thb history cv', num2str(cv),'.mat'], ...
        ['eval_',data_name,'_thb history ',save_date,' cv', num2str(cv),' 2round.mat']);
end
...
for cv = 1:num_Exp
    trains = [1:cv-1, cv+1:num_Exp]; 
    load(['.\eval_',data_name,'_thb history ',save_date,' cv', num2str(cv),' 2round.mat'],'list_seq',...
...
%     Table_train = mean(Table_train);
    Table_train = end_history(end-3:end);
    list_Table{cv} = [best_param, Table_test, Table_train];
end
Table = cell2mat(list_Table);
Table_time_ext=[Table;nanmean(Table,1);nanstd(Table,1,1)];
save(['eval_',data_name,'_thb ',save_date,' cv 2round.mat'],'Table_time_ext');
