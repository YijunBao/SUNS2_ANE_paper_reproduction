...
%% 
path_name = 'D:\ABO\20 percent 200\';
...
%     nam = get_fullname('E:\1photon-small\c25_59_228.h5');
    ...
    ring_radius = 2*gSiz;  % when the ring model used, it is the radius of the ring used in the background model.
    ...
    dmin = gSiz/3;       % minimum distances between two neurons. it is used together with merge_thr
...
%     Masks3 = permute(Masks3,[2,1,3]);
    ...
    list_time(eid) = seconds(process_time{3}-process_time{2});
    
    %% move the final results
    child = [path_name,Exp_ID,'_source_extraction\'];
    saved_files = dir([child,'\frames*\LOGS*\*Dec*.mat']);
    datenum = [saved_files.datenum];
    [val,ind] = max(datenum);
    saved_file = saved_files(ind);
    movefile([saved_file.folder,'\',saved_file.name], [dir_save,Exp_ID,'_result.mat']);
    save([dir_save,Exp_ID,'_Masks.mat'],'Masks3');
