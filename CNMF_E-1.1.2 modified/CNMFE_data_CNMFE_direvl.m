...
list_params.th_binary = [0.2, 0.3, 0.4, 0.5];
...
init_ind_param = [1, 3, 2, 7, 4, 6, 8, 4, 1]'; 
        ...
        %                 nk = list_nk; % 3;             % detrending the slow fluctuation. usually 1 is fine (no detrending)
                ...
                %         min_pnr = 8; % 8;       % minimum peak-to-noise ratio for a seeding pixel
                    ...
                    %     Masks3 = permute(Masks3,[2,1,3]);
                        [Recall(eid), Precision(eid), F1(eid)] = GetPerformance_Jaccard(dir_GT,Exp_ID,Masks3,0.5);
                        used_time(eid) = seconds(process_time{4}-process_time{2});
...
%                         movefile(fullfile(saved_file.folder,saved_file.name), fullfile(dir_save,dir_sub,[Exp_ID,'_result.mat']));
                ...
                if mean(F1) > mean(best_F1)
                    ...
                    history = [history; temp_param', mean(Table_time)];
                    save(['eval_',data_name,' history.mat'],'best_Recall','best_Precision','best_F1',...
                        'best_time','best_ind_param','ind_param','list_params','history')
                elseif mean(F1) < mean(best_F1)
                    break
                end
            end
...
%%
movefile(['eval_',data_name,' history.mat'], ['eval_',data_name,char(datetime),'.mat']);
